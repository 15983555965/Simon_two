...
import com.common.modules.cache.HttpCache;
import com.common.modules.cache.HttpCacheHandler;
import com.common.modules.location.LocationServer;
...
public abstract class Api extends BaseModule {
    ...
    private static int sReqCount;//上报接口最大请求次数
    ...
    public static void registerGlobalConfig(Context app, final ApiConfiger configer,
        ...
        if (NetworkState.available()
                && UIUtils.isRunInMainThread()) {
            Logger.d("API", "init sync api from remote");
            final Api api = new EmptyApi();
            api.mIsRegisterProcess = true;
            api.addParams("app", "wecook", true)
                    ...
                    .setApiCallback(new ApiCallback<ApiConfiger>() {
                        @Override
...
//            api.serverUrl = "http://114.215.120.225:8080/"+"v3";
    ...
    public static <T extends Api> T get(Class<T> instanceCls) {
        ...
        if (instanceCls != null) {
            synchronized (Api.class) {
                ...
                register(UIUtils.getContext(), api, mGlobalConfig);
            }
        }
        return (T) api;
    }
    ...
    public static void setRequestTag(String requestTag) {
        ...
        Logger.d("API", "set api request tag : " + requestTag);
    }
    ...
    public static void stopAllRequest(String requestTag) {
        if (!StringUtils.isEmpty(sApiRequestTag)
            ...
            Logger.d("API", "stop api request tag : " + requestTag);
        }
    }
