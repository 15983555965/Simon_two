...
public class RedPacketHelp {
    ...
    public static double[] clipRedPacket(double total, int partCount){
        ...
        float[] percentes=new float[partCount];
        fillPercentes(ints,percentes);
        double[] result = clip(total, percentes);
        return result;
    }

    private static double[] clip(double total, float[] percentes) {
    ...
    private static void fillPercentes(int[] ints, float[] percentes) {
        int totalRandom=getTotal(ints);
        for (int i=0;i<ints.length;i++){
            percentes[i]=(float) ints[i]/(float)totalRandom;
        }
    }
